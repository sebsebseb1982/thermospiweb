...
public class DataServiceImpl implements DataService {
	...
	// public Collection<ThermostatSetPoint> getLastThermostatState(int
	// lastUnit, int lastAmount) {
	// Date now = new Date();
	//
	...
	// session.createCriteria(ThermostatSetPoint.class).add(Restrictions.gt("date",
	// calendar.getTime())).addOrder(Order.asc("date"));
	...
	// session.createCriteria(ThermostatSetPoint.class).addOrder(Order.desc("date"))
	...
	// end.setDate(now);
	// end.setState(thermostatStates.get(thermostatStates.size() -
	// 1).isState());
